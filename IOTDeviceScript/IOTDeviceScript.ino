...
const char ssid[] = "JOANMA"; // TODO cambiar por el nombre de la red WiFi
...
bool checkMeasures(float t, float h, float l) {
  // Se comprueba si ha habido algún error en la lectura
    if (isnan(t) || isnan(h)) {
      ...
      Serial.println("Error obteniendo los datos del foto resistor");
      return false;
    }
    return true;
}
...
void displayMeasures() {
  ...
  if (checkMeasures(temp, humi)) {
      measures.concat("T: ");
      measures.concat(temp);
      measures.concat(" H: ");
      measures.concat(humi);    
  }else {
    measures = "T:    H:   ";
    }                                                                                                                                                               
  lcd.print(measures);
...
void displayMessage(String message) {
  if (!message.equals("OK")) {
    ...
    lcd.setCursor(0,1);
    lcd.print(message);
    if(message.indexOf("luminosidad") > 0) {
